...
class Season(object):
    ...
    def get_teams(self):
        """
        :return: all teams and their results data for using in next season if it will be called successively with that
        """
        return self.teams
    ...
    def run_UEFA_Champions_League(self):
        ...
        UEFA_CL_tourn.run() # TODO FIX NotImplementedError FOR RUN 2nd SEASON IN SESSION
        # id if played CL is defined in run method
        CL_id_tourn = UEFA_CL_tourn.getID()

        # use CL data to seed loosers of some rounds of CL to EL
        # get group_third_places to seed them to League Europa
        self.CL_EL_seeding = UEFA_CL_tourn.get_CL_to_EL_seeding()

        if not (self.CL_EL_seeding["Qualification 3"] or self.CL_EL_seeding["Qualification 4"]):
            # normally this should never happens

            # # get Qual 3 and Qual 4 loosers indexes to seed them to League Europa
            # self.CL_EL_seeding = UEFA_CL_tourn.getNet()
...
def Test(*args, **kwargs):
    ...
    # RUN SEASONS
    # members is a cached data from previous seasons stored in RAM for quick access if next season will be called in
    # the same session
    members = None
    for t_ in range(t_num):
        print "\n=======================================\nTEST SEASON %s" % (t_ + 1)
        tst_season = Season(members)
        tst_season.run()
        members = tst_season.get_teams()
